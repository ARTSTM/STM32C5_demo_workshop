...
To build the application we can save all active files.
...
- on the bottom window an additional tab called STM32CUBE RTOS. It will display data about FreeRTOS components once we pause the code execution (there is no live monitor of FreeRTOS state and its components).

To test it.
Start an application, after the while pause it and see an effect on the board and in STM32CUBE RTOS window.

That's all for this part. 

We can stop debug session, by pressing STOP icon.
